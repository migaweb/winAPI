...
lpSystemName = None
lpName = "SeRestorePrivilege"
lpLuid = LUID()
response = a_handle.LookupPrivilegeValueW(lpSystemName, lpName, ctypes.byref(lpLuid))

# Handle an Error
if response > 0:
    print("[INFO] Found the LUID! Token: LUID high: {0}, LUID low: {1}".format(lpLuid.HighPart, lpLuid.LowPart))
else:
...
if response > 0:
    ...
    print("[ERROR] Could Not run priv check! Error Code: {0}".format(k_handle.GetLastError()))
